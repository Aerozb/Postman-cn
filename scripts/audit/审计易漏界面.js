...
function resolveOutPath(value) {
  const requested = value || "postman-targeted-surfaces.json";
  const hasDirectory = path.isAbsolute(requested) || requested.includes("/") || requested.includes("\\");
  let resolved = hasDirectory
    ? path.resolve(requested)
    : path.resolve(__dirname, "..", "..", "..", "_generated", requested);
  if (!hasDirectory && !path.extname(resolved)) {
    resolved += ".json";
  }
  return resolved;
}
...
const domScript = (scope = "all") => `(() => {
 ...
 for(const {r,t} of roots){const overlays=[...r.querySelectorAll(overlaySel)].filter(v), inside=e=>overlays.some(o=>o===e||o.contains(e));for(const e of r.querySelectorAll("*")){if(/^(SCRIPT|STYLE|NOSCRIPT|TEMPLATE)$/.test(e.tagName)||S==="overlay"&&!inside(e))continue;
   ...
   if(v(e))for(const x of e.childNodes||[])if(x.nodeType===3)add(x.nodeValue,"text");for(const a of A)if(e.hasAttribute&&e.hasAttribute(a))add(e.getAttribute(a),"attribute",a);
...
async function main(){
 const out=resolveOutPath(arg("--out",null)), delay=Number(arg("--delay-ms","500"));
 ...
 const report={generatedAt:new Date().toISOString(),target:{title:target.title,url:target.url},summary:{snapshots:shots.length,actions:actions.length,successfulActions:actions.filter(x=>x.ok).length,findings:merged.size},actions,findings:[...merged.values()].sort((a,b)=>b.count-a.count),snapshots:shots};fs.mkdirSync(path.dirname(out),{recursive:true});fs.writeFileSync(out,JSON.stringify(report,null,2));const summary={out,summary:report.summary,actions,top:report.findings.slice(0,40).map(x=>x.text)};console.log(`易漏界面审计完成：执行 ${report.summary.actions} 次探测，发现 ${report.summary.findings} 条候选，报告已写入 ${out}`);if(flag("--details"))console.log(JSON.stringify(summary,null,2));
}

function selfTest(){
 const generated=domScript("all"),expected=path.resolve(__dirname,"..","..","..","_generated","自检报告.json"),checks=[[/\be\.value\b/.test(generated),false],[/input-value/.test(generated),false],[resolveOutPath("自检报告"),expected]];
 const failed=checks.filter(([actual,wanted])=>actual!==wanted);if(failed.length)throw new Error(`自检失败，共 ${failed.length} 项不符合预期。`);
 if(flag("--details"))console.log(JSON.stringify({ok:true,checks:checks.length},null,2));else console.log(`易漏界面审计脚本自检通过，共 ${checks.length} 项。`);
}

Promise.resolve().then(()=>flag("--self-test")?selfTest():main()).catch(e=>{const message=String(e&&e.message||e).replace(/\s+/g," ").trim();if(flag("--details"))console.error(JSON.stringify({ok:false,error:message,stack:e&&e.stack||null},null,2));else console.error("易漏界面审计失败，请确认 Postman 已启动；可使用 --details 查看详细信息。");process.exitCode=1});
