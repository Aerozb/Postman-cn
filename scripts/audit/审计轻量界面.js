...
async function collect(cdp, step) {
  return evaluate(cdp, `(() => {
    ...
    function allowedLine(value) {
      ...
      // 模型名称（如 GPT-5.6 Sol）和版本号是产品标识，不属于漏翻。
      if (/^gpt-\\d+(?:\\.\\d+)?(?:\\s+[a-z][a-z0-9.-]*)+$/i.test(value)) return true;
...
main().catch((error) => {
  ...
  if (SHOW_DETAILS) {
    ...
    console.error("轻量界面审计失败，请确认 Postman 已启动；可使用 --details 查看详细信息。");
  }
  process.exit(1);
});
