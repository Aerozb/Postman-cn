...
const samples = {
  ...
  "translationProbeTargets": [
    ...
    "Toggle bottom bar Ctrl+`",
    "Toggle right sidebar Ctrl+Alt+\\",
  ...
  "translationProbeExpectations": [
    ...
    [
      "capture cookies using Interceptor",
      "使用 Interceptor 捕获 Cookie"
    ],
    [
      "e.g. read:org",
      "例如 read:org"
    ...
    [
      "Drag and drop or choose a .proto file from your local system.",
      "拖放或从本地系统选择 .proto 文件。"
    ],
    [
      "while resolving \"import\" directives.",
      "解析“import”指令时。"
    ],
    [
      "团队成员 are part of example-workspace team",
      "团队成员属于 example-workspace 团队"
    ],
    [
      "Postman 的 Enterprise Trial",
      "Postman 企业试用版"
    ...
    [
      "Select a dataset and view in Test data to see data here.",
      "在“测试数据”中选择数据集和视图，即可在此处查看数据。"
    ],
    [
      "Global variables for a workspace are a set of variables that are always available within the scope of that workspace. They can be viewed and edited by anyone in that workspace.",
      "工作区的全局变量是一组在该工作区范围内始终可用的变量。工作区中的任何人都可以查看和编辑这些变量。"
    ],
    [
      "Global variables for a workspace are a set of variables that are always available within the scope of that workspace. They can be viewed and edited by anyone in that workspace.",
      "工作区的全局变量是一组在该工作区范围内始终可用的变量。工作区中的任何人都可以查看和编辑这些变量。"
    ...
    [
      "A comprehensive view of your organization analytics and metrics",
      "全面查看组织的分析数据和指标"
    ],
    [
      "A comprehensive view of your organization 分析 and metrics",
    ...
    [
      "Top 5 active users by API requests sent",
      "按 API 请求发送量排名前 5 的活跃用户"
    ],
    [
      "Top 5 collections by API requests sent",
      "按 API 请求发送量排名前 5 的集合"
    ],
    [
      "Top 5 workspaces by API requests sent",
      "按 API 请求发送量排名前 5 的工作区"
    ],
    [
      "Total number",
      "总数"
    ...
    [
      "Unlock with native password manager",
      "使用本机密码管理器解锁"
    ],
    [
      "Run on Cloud (static IP)",
      "在云端运行（静态 IP）"
    ],
    [
    ...
    [
      "Restart to apply tool changes",
      "重启以应用工具更改"
    ],
    [
      "Requests come from static IPs you can allowlist in your firewall.",
      "请求来自静态 IP，你可以将这些 IP 加入防火墙白名单。"
    ],
    [
      "More about native Git support on Postman",
      "了解 Postman 的原生 Git 支持"
    ],
    [
      "Turned off by your organization’s Postman installation settings.",
      "已由你所在组织的 Postman 安装设置关闭。"
    ],
    [
      "1 Folder",
      "1 个文件夹"
    ...
    [
      "These modules can't load in the cloud: fs, node:net, @example/lib. Use pm.require() for third-party libraries, remove blocked built-ins (like fs), then push again.",
      "以下模块不支持在云端加载：fs, node:net, @example/lib。请用 pm.require() 引入第三方库，并移除受限的内置模块（如 fs），然后重新推送。"
    ],
    [
      "These modules can’t load in the cloud: fs. Use pm.require() for third-party libraries, remove blocked built-ins (like fs), then save again.",
      "以下模块不支持在云端加载：fs。请用 pm.require() 引入第三方库，并移除受限的内置模块（如 fs），然后重新保存。"
    ]
  ],
...
function evaluateTranslations(localizer, samples) {
  const outputs = samples.translationProbeTargets.map(input => ({ input, output: localizer.translate(input) }));
  return {
    untranslated: outputs.filter(item => item.output === item.input).map(item => item.input),
    englishHits: outputs.map(item => {
      const hits = samples.residualEnglish.filter(needle => String(item.output || "").includes(needle));
      return hits.length ? { ...item, hits } : null;
    }).filter(Boolean),
    unexpectedTranslations: samples.translationProbeExpectations.map(([input, expected]) => {
      const output = localizer.translate(input);
      return output === expected ? null : { input, expected, output };
    }).filter(Boolean),
    preservationFailures: samples.translationPreservationTargets.map(input => {
      const output = localizer.translate(input);
...
module.exports = { samples, evaluateTranslations };
