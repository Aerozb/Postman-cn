...
| 序号 | 操作 | 说明 |
|---:|---|---|
| `1` | 安装汉化 | 打补丁、关闭自动更新并验证，成功后清理旧版本；直接回车也执行此项。 |
...
菜单安装成功后会清理同一官方安装目录中的旧版本和旧安装包，保留当前版本、英文原版备份及用户数据；安装或验证失败时保留旧版本。维护者直接运行 `install` 默认保留旧版，追加 `-CleanOldVersions` 可启用同样的清理。
...
「设置 > 更新」页里还有第二个开关——**汉化版本更新检查**，默认**开启**。它每 6 小时问一次本项目的 GitHub 发布页，有新版就在右下角浮个小提示，点「打开发布页」跳转下载。
